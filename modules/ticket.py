...
class support(commands.Cog):
	...
	async def new(self, ctx, *, desc: typing.Optional[str] = "지정되지 않았어요."):
			...
			if str(ctx.author.id) in ctx.channel.topic or staffs in ctx.author.roles:
				...
				try:
					...
					if str(reaction.emoji) == "<:cs_yes:659355468715786262>":
						...
						await ctx.channel.edit(name=ctx.channel.name.replace("지원", "닫힘"), category=deprecated)
			...
			if str(ctx.author.id) in channel.topic or staffs in ctx.author.roles:
				...
				try:
					...
					if str(reaction.emoji) == "<:cs_yes:659355468715786262>":
						...
						await channel.edit(name=ctx.channel.name.replace("닫힘", "지원"), category=activated)
