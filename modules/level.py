...
class level(commands.Cog):
	...
	async def on_message(self, msg):
		if msg.author.bot:
			...
			return
		elif msg.content.startswith('_') or msg.content.startswith("라비님"):
			return
		else:
			...
			if not rows:
				cur.execute(f"INSERT INTO rank(user, exp, level, next, cooldown) VALUES({msg.author.id}, {add}, 0, 500, 'true')")
				conn.commit()
				conn.close()
	...
	async def rank(self, ctx, members: commands.Greedy[discord.Member]):
		...
		rows = cur.fetchall()
		if rows:
			embed = discord.Embed(title=f"{str(member)}님의 경험치 정보", description=f"모든 경험치는 1분에 1회만 적립돼요! 적립이 안된다면 {dev.mention}에게 문의해주세요!", color=0xFFFCC9)
