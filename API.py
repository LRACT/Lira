...
async def on_message(msg):
	...
	if msg.guild.id == 702880464893116518:
		...
		if msg.content.startswith("r.") or msg.content.startswith(f"<@{bot.user.id}> ") or msg.content.startswith(f"<@!{bot.user.id}> "):
			...
			if manager not in msg.author.roles or "[ 초대 승인됨 ]" not in msg.channel.topic or msg.channel.topic is None:
				await msg.delete()
			...
			if manager not in msg.author.roles or msg.channel.topic is None or "[ 링크 승인됨 ]" not in msg.channel.topic:
				await msg.delete()
